...
class TestFineTuningCancel:
    ...
    @pytest.mark.respx(base_url=base_url)
    def test_cancel_json_requires_quiet(self, respx_mock: MockRouter, cli_runner: CliRunner) -> None:
        running = {**_FT_RETRIEVE_BODY, "status": "running"}
        respx_mock.get("/fine-tunes/ft-1").mock(return_value=httpx.Response(200, json=running))
        # --json without --non-interactive still hits the confirmation prompt (stdin is empty).
        with pytest.raises(EOFError):
            cli_runner.invoke(["fine-tuning", "cancel", "ft-1", "--json"])
    ...
    def test_cancel_not_cancellable_json(self, respx_mock: MockRouter, cli_runner: CliRunner) -> None:
        ...
        assert result.exit_code == 1
        assert result.out_out == ""
        assert "Training is not currently cancellable" in result.err_out
