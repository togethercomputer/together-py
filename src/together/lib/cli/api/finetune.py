...
from ...utils import parse_timestamp, finetune_price_to_dollars


@click.group(name="fine-tuning")
@click.pass_context
def fine_tuning(_ctx: click.Context) -> None:
...
def download(
    ...
    response = client.fine_tune.download(ft_id=fine_tune_id, output=output_dir, checkpoint_step=checkpoint_step)
