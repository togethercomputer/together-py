...
async def create(
    ...
    learning_rate: Annotated[float, Parameter(name=["--learning-rate", "-lr"], help="Learning rate")] = 1e-5,
    ...
    lora_r: Annotated[int, Parameter(help="LoRA adapters' rank")] = 8,
    lora_dropout: Annotated[float, Parameter(help="LoRA adapters' dropout")] = 0,
    lora_alpha: Annotated[float, Parameter(help="LoRA adapters' alpha")] = 8,
    ...
    train_on_inputs: Annotated[
        Optional[bool | Literal["auto"]],
        Parameter(
            converter=bool_or_auto_converter,
    ...
    *,
    ...
    if lora is None:
        ...
        for arg in default_values:
            ...
            if arg_source == ParameterSource.DEFAULT:  # type: ignore[attr-defined]
    ...
    finetune_price_estimation_result = await config.client.fine_tuning.estimate_price(
    ...
    if not confirm:
        ...
        if resp and resp != "y" and resp != "yes":
            return
    response = await config.client.fine_tuning.create(**training_args, verbose=True)
