...
class JigConfig:
    ...
    def __post_init__(self) -> None:
        if err := validate(self, type(self)):
            raise Exception(f"Invalid {self._path}: {err}")
            # TODO:
    ...
    def find(cls, config_path: str | None = None, init: bool = False) -> JigConfig:
        ...
        if init:
            return cls()
        raise Exception("No pyproject.toml or jig.toml found, use --config to specify a config path")
        # TODO:
...
def _files_to_copy(config: JigConfig) -> list[str]:
    ...
    if config.image.auto_include_git:
        try:
            if _run(["git", "status", "--porcelain"]).stdout.strip():
                raise Exception("Git repository has uncommitted changes: auto_include_git not allowed")
                # TODO:
    ...
    if "." in files:
        raise Exception("Copying '.' is not allowed. Please enumerate specific files")
        # TODO:
