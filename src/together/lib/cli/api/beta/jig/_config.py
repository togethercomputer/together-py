...
class Config:
    ...
    def get_deploy_data(self) -> dict[str, Any]:
        deploy_data = {
            ...
            "environment_variables": [{"name": k, "value": v} for k, v in config.deploy.environment_variables.items()],
        }
...
class State:
    ...
    def get_secret_environ(self) -> list[dict[str, str]]:
        return [{"name": name, "value_from_secret": secret_id} for name, secret_id in self.secrets.items()]
