...
async def _list_ab_experiments(id: str, *, config: CLIConfigParameter) -> list[AbExperiment]:
    try:
        response = await config.client.beta.endpoints.ab_experiments.list(id)
    except PermissionDeniedError:
        # Some access proxies allow the canonical endpoint path but block optional
        # experiment subresources. Basic endpoint retrieval should still work.
        return []
...
async def _list_shadow_experiments(id: str, *, config: CLIConfigParameter) -> list[ShadowExperiment]:
    try:
        response = await config.client.beta.endpoints.shadow_experiments.list(id, include_targets=True)
    except PermissionDeniedError:
        return []
    return response.data
