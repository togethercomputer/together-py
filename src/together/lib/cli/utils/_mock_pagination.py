...
AfterParameter = Parameter(name="after", help="The cursor to start from.")
...
def mock_pagination(
    ...
    """
    index_of_start = (
        next((i for i, item in enumerate(data) if getattr(item, cursor_field) == cursor), 0) if cursor else 0
    )

    items_to_display = data[index_of_start : index_of_start + page_size]

    next_index = index_of_start + page_size
