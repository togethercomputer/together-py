    ...
    async def prompt(self, field: str) -> str | bool:
        ...
        if self.choices is not None:
            return await questionary.select(
                self.message or field, choices=self.choices, style=custom_style_fancy, show_selected=True
            ).unsafe_ask_async()

        if self.type == "confirm":
            return await questionary.confirm(self.message or field, style=custom_style_fancy).unsafe_ask_async()

        return await questionary.text(
            self.message or field, instruction="\n→", style=custom_style_fancy #, validate=NameValidator
        ).unsafe_ask_async()
