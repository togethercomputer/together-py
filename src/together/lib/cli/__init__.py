...
from cyclopts import App, Group, Parameter, UnknownOptionError, MissingArgumentError
...
# TODO: Add flush_pending_events back in
# TODO: Add auto_track_command for all the commands
...
async def launcher(
    *tokens: Annotated[str, Parameter(show=False, allow_leading_hyphen=True)],
    ...
    json: Annotated[
        Optional[bool], Parameter(group=common_parameters, negative=(), help="Output the response in JSON format")
    ] = False,
) -> None:
    ...
    (parsed_command, explicit_args, is_beta_command) = parse_command_and_flags([*remaining])
    ...
    async def run_command() -> None:
        try:
            ...
            console.print(
                f"[dim yellow]'{e.token.value}' is not a valid option for this command. It will be ignored.[/dim yellow]"
            )

            # Rerun the command without the unknown option
            remaining.remove(e.token.value)
            await run_command()
...
beta_app = app.command(App(name="beta", help="Beta API commands", show=False))
...
def _maybe_auto_install_completion() -> None:
    ...
    try:
        ...
        sentinel.touch()
        print(
            "Together shell command completion installed. Restart your shell or source your rc file to start using it.",
            file=sys.stderr,
        )
    except Exception:
        pass
...
def main() -> None:
    Thread(target=_maybe_auto_install_completion, daemon=True).start()
    app.meta()
